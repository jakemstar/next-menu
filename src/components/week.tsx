...
export default function Week() {
    ...
    const events = user ? user[0]?.events : undefined;
    console.log(data)
    ...
    return (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4 pt-6">
            {contentDaysOfTheWeek.map((day, idx) =>
                (<RecipeCard key={day.name} href={day.name} isCurrent={idx === dayjs().day()} slashDate={day.slashDate} title={day.recipe?.title || "null"} description={day.recipe?.desc || "No meal set for this day yet."} picture={day.recipe?.imageUrl || "/utensilplate.png"} buttonText={day.recipe ? "Edit recipe" : "Select recipe"} />)
            )}
        </div>
    )
}
...
export function RecipeCard(recipeCardProps: recipeCardProps) {
    return (
        <div className={classNames(recipeCardProps.isCurrent ? 
            ...
            <div>
                <h4 className={recipeCardProps.isCurrent ? "font-bold text-3xl text-center pt-2 text-slate-50 dark:text-slate-900" : "font-bold text-3xl text-center pt-2"}>{recipeCardProps.href}</h4>
                <a href="#" className="">
                    <Image className="rounded-lg m-auto mt-4" src={`${recipeCardProps.picture}`} alt="" height={150} width={150} />
                </a>
            </div>
            <div className="px-5 pt-4 flex flex-col">
                <h5 className={classNames(recipeCardProps.isCurrent ? "dark:text-slate-900 text-slate-50" : 
                "text-slate-900 dark:text-white", "mb-2 text-2xl font-bold tracking-tight")
                }>{recipeCardProps.title}</h5><p className={classNames(recipeCardProps.isCurrent ? 
                    "text-slate-50 dark:text-slate-800" : 
                    "text-slate-700 dark:text-slate-50", "mb-3 font-normal")
                }>{recipeCardProps.description}</p>
                <div className="flex justify-between align-middle">
                    <Link href={`/day/${recipeCardProps.href || ""}`} className={classNames(recipeCardProps.isCurrent ? 
                        ...
                        {recipeCardProps.buttonText}
                        <svg aria-hidden="true" className="w-4 h-4 ml-2 -mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                    </Link>
                    <p className={classNames(recipeCardProps.isCurrent ? "text-slate-50 dark:text-slate-900" : "text-slate-900 dark:text-slate-50", "text-right text-bold")}>{recipeCardProps.slashDate}</p>
                </div>
            </div>
        </div>
    )
